...
const Settings: React.FC = () => {
  ...
  const handleNicknameConfirm = () => {
    ...
    UserService.doSetNickname(newNick, "/infra/user/nickname", store).then(
      (res) => {
        if (ResponseHandler.responseSuccess(res)) {
          UserService.loadCurrUser(true, readConfig("refreshUserUrl")).then(
            (res) => {
              const userInfoJson = localStorage.getItem("userInfo");
              if (userInfoJson) {
                const uInfo: UserModel = JSON.parse(userInfoJson);
        ...
        <div id="userinfo">
          <div className="card" style={{ marginBottom: "20px" }}>
            ...
            <div className="card-body row">
              <div className="col">
                <div>
                  <span className="user-info">{t("nickname")}:</span>
                </div>
                <div>
              ...
              <div className="col">
                <div>
                  <span className="user-info">{t("vip_expire")}:</span>
                </div>
                <div>
                  <span className="user-info">
                    {userInfo ? UserProfile.getVipExpiredTime(userInfo) : "--"}
                  </span>
                </div>
                <div></div>
              </div>
